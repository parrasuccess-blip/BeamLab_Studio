...
assert.ok(start>=0&&stop>start,'Production registry missing');
const context=vm.createContext({TextEncoder,TextDecoder,setTimeout,clearTimeout,AbortController,console});
new vm.Script(html.slice(start,stop)+';globalThis.productionLoad=load;').runInContext(context);
const load=context.productionLoad;

const {prepareMoving,influenceLine,envelope,envelopeCsv,validateConfig}=load('engine/moving');
const {shearProfile}=load('engine/shear');
