...
**STATUS: 4.1.5 candidate implementation complete / numerical validation passed / final browser validation pending.**
...
- Further candidate polish: active standalone lessons now focus their task at the top of the viewport and hide the inactive catalogue/progress controls; those return on exit. Added typographic moment units throughout visible Studio views/exports, a real WinAnsi middle dot in PDF text, and exact 4.1.5 build/tutor/release identity. Hosted 4.1.4-labelled preview confirmed edit 6→8 m, 40 kN·m moment, retained history and original model after lesson return, and solver-derived 6 m / 20 kN Show Why. Latest local `npm test`: **1,297 passed**. Production remains 4.1.4 until complete CI and exact public verification.
- Next: checkpoint source milestones to this branch, run the complete browser suite and review hosted desktop/phone screenshots; merge and verify the exact public artifact only after all gates pass. No new solver, design-code claims or numerical changes intended.
