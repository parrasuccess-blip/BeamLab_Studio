...
function teaching(m, a, x, level = 'all') {
    ...
    return `<div class="teach-head"><span class="eyebrow">SHOW WHY / x = ${(0,common_1.fmt)(e.x)} m</span><span class="tag">From the solved model</span></div><h3>${(0,common_1.esc)(e.title)}</h3><p class="show-why-prompt">Click a diagram or set Inspect x to explain a different position. The same x is used across the structure, shear and moment views.</p><div class="show-why-steps">${e.steps.map(step => `<article><span>${(0,common_1.esc)(step.label)}</span><b>${(0,common_1.esc)(step.equation)}</b><strong>${(0,common_1.esc)(step.value)}</strong><p>${(0,common_1.esc)(step.detail)}</p></article>`).join('')}</div><details class="show-why-details"><summary>Actions, supports &amp; one-sided values here</summary><div class="explanation-detail">${e.lines.map(line => `<p>${(0,common_1.esc)(line)}</p>`).join('')}</div></details><p class="show-why-boundary">This explanation is generated deterministically from the solved model. It describes elastic response, not member capacity or code compliance.</p>`;
}
