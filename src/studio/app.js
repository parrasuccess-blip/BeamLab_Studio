...
function renderDesignStudio() {
    ...
    <section class="design-card design-step-card design-step-1"><span class="eyebrow">SECTION & MEMBER CONTEXT</span><h3>${(0,common_1.esc)(r.section.label)}${r.section.piecewiseEI?' + piecewise EI':''}</h3><div class="design-section-grid"><article><span>E / base</span><b>${(0,common_1.fmt)(r.section.E_GPa,3)} GPa</b></article><article><span>Ix / base</span><b>${Number(r.section.I_mm4).toExponential(3)} mm⁴</b></article><article><span>Area / base</span><b>${(0,common_1.fmt)(r.section.A_mm2,1)} mm²</b></article><article><span>EI zones</span><b>${r.section.stiffnessZones?.length || 0}</b></article></div>${r.section.piecewiseEI?`<div class="design-warning">EI multipliers change analysis stiffness only. Verify local section geometry and that every entered capacity applies to the relevant zone; BeamLab does not infer this from EI.</div>`:''}<p class="design-source-note">${m.section.catalogue?'Catalogue geometry/area/Ix comes from the existing BeamLab InfraBuild reference library. The library is not a design-capacity database.':'Current section properties come from the BeamLab section model.'}</p></section>
