...
export default async function handler(req, res) {
  if (req.method === 'GET') return res.status(200).json({ message: 'Success', release: '4.1.4', configured:!!process.env.OPENAI_API_KEY });
  ...
  try {
    ...
    return res.status(200).json({ answer, solverAuthoritative: true, release: '4.1.4' });
  } catch {
